    </div><!-- end container -->
    <div id="footer">
	<img src="<?php bloginfo('stylesheet_directory'); ?>/images/logo-UvA.svg" alt="logo University of Amsterdam" />
	<img src="<?php bloginfo('stylesheet_directory'); ?>/images/logo-fordfoundation.svg" alt="logo Ford Foundation" />
	<img src="<?php bloginfo('stylesheet_directory'); ?>/images/logo-isocfoundation.svg" alt="logo ISOC Foundation" />
	<img src="<?php bloginfo('stylesheet_directory'); ?>/images/logo-omidyar-network.svg" alt="logo Omidyar Network" />
    </div>
    <canvas id="canvas" width="600" height="600"></canvas>
    <script type="text/javascript" src="<?php bloginfo('stylesheet_directory'); ?>/js/vivus.min.js"></script>
    <script type="text/javascript" src="<?php bloginfo('stylesheet_directory'); ?>/js/draw_antenna.js"></script>
    <script type="text/javascript" src="<?php bloginfo('stylesheet_directory'); ?>/js/draw.js"></script>
    <script type="text/javascript" src="<?php bloginfo('stylesheet_directory'); ?>/js/init.js"></script>
    <?php wp_footer();  ?>
</body>
</html>
